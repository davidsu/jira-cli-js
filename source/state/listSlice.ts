...
const setList = (state, { payload }) => {
  state.displayList = payload.issues.map(v => makeDisplayRow(payload, v))
...
export const listSlice = createSlice({
  ...
  extraReducers: builder => {
    builder.addCase(fetchList.fulfilled, setList)
  },
})

export const fetchList = createAsyncThunk<any, string>('list/fetch', async (jql, thunkAPI) => {
  const { server, cache } = search(`${jql}&fields=${fields}`)
  cache.then(data => thunkAPI.dispatch(dataFetched(data)))
  return server
})

// Action creators are generated for each case reducer function
export const { dataFetched } = listSlice.actions
