...
interface Props {
  ...
  onCombo?: (input: string, key: Key) => void
}
...
const TextInput: FC<Props> = ({
  ...
  useInput(
    (input, key) => {
      ...
      if (key.ctrl && input === 'a') {
        setState({ cursorOffset: 0, cursorWidth })
        return
      }
      if (key.ctrl && input === 'e') {
        setState({ cursorOffset: originalValue.length, cursorWidth })
        return
      }
      if (key.ctrl && input === 'u') {
        onChange('')
...
export const UncontrolledTextInput: FC<UncontrolledProps> = ({ initialValue = '', ...props }) => {
